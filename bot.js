...
client.on('ready', function(){
    ...
    setInterval(function (){
        ...
        i = i+j;
        client.user.setGame(setGame[i],`http://www.twitch.tv/Xers`);
    }, ms);
...
client.login('NjUyNjIwNDQ5NzY2NzAzMTE4.Xeszqg.vttuo9b3SUm5yDf_XSiHQIU_cMw');
client.on('message', message => {
...
client.on('message', message => {
    if(message.content == '^vip-servers') {
             if(!message.author.id === '445341233641160704') return;
    var gimg;
    var gname;
    var gmemb;
    var gbots;
...
if (message.author.id !== '445341233641160704') return message.reply('** هذا الأمر فقط لصاحب البوت و شكراًً **')
client.user.setGame(argresult);
